...
const ChatComponent = ({ reportData }: Props) => {
  const { messages, input, handleInputChange, handleSubmit, isLoading, data, error } =
    useChat({
        ...
        >
          <div className="relative">
            ...
            <Button
              ...
              type="submit"
              size="sm"
              className="absolute right-2 top-1/2 -translate-y-1/2 h-9 w-9 rounded-xl bg-black hover:bg-zinc-900 disabled:bg-gray-300 dark:disabled:bg-zinc-700 transition-all duration-200 shadow-lg"
            >
              {isLoading ? (
